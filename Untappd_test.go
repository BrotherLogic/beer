...
func (fetcher stubFailFetcher) Fetch(url string) (*http.Response, error) {
	var resp *http.Response = &http.Response{}
	var err error = errors.New("Built to fail")
	return resp, err
}

func TestGetBeerName(t *testing.T) {
	beer_name := GetBeerName(7936)
	if beer_name != "Firestone Walker Brewing Company - Parabola" {
...
func TestGetBeerPage(t *testing.T) {
	...
	beer_page := GetBeerPage(fetcher, converter, 7936)
	if !strings.Contains(beer_page, "Firestone") {
...
func TestGetBeerPageFailHttp(t *testing.T) {
	...
	var converter ResponseConverter = mainConverter{}
	beer_page := GetBeerPage(fetcher, converter, 7936)
	if !strings.Contains(beer_page, "Failed to retrieve") {
...
func TestGetBeerPageConvertHttp(t *testing.T) {
	...
	var converter ResponseConverter = stubFailConverter{}
	beer_page := GetBeerPage(fetcher, converter, 7936)
	if !strings.Contains(beer_page, "Failed to retrieve") {
		t.Errorf("Beer page retrieve did not fail\n%q\n", beer_page)
	}
...
func TestConvertPageToName(t *testing.T) {
	...
	beer_name := ConvertPageToName(mock_beer_page, unmarshaller)
	if beer_name != "Firestone Walker Brewing Company - Parabola" {
